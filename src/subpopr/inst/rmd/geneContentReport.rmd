...
```{r}
x<-corr %>%
  filter(statSigLgl & correlationR > rStatCutoff,.preserve = T)
if(nrow(x)==0){
...
f <- function(x,pos){ filter(x,X1 %in% sigGeneFams) }
...
#   
  p <- corrsToPlot %>% filter(geneFamily == geneFam,.preserve = T) %>%
