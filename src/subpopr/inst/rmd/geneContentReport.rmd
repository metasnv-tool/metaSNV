...
```{r}
data.frame(files=c(pathCorrP,pathCorrS,pathClusterGenesDf))
```
...
if(ncol(df) == 4 & all(c("pearson","spearman") %in% colnames(df) ) ){ # has spearman and pearson
  ...
  if(nrow(dff)<50){
    p+ggrepel::geom_text_repel(size=3)
  }else{
    p
  }
}
...
```{r}
# replaced with table above
# corrSubspeciesSpecific %>% 
#   head(n=100) %>% 
#   select(geneFamily,cluster,method,correlationR,geneIsCorrelated,geneIsNotCorrelated) %>% 
#   mutate(geneIsCorrelated = case_when( geneIsCorrelated & !geneIsNotCorrelated ~ "Gene is correlated",
#                                        !geneIsCorrelated & geneIsNotCorrelated ~ "Gene is not correlated",
#                                        TRUE ~ "unclear")) %>% 
#   ggplot(aes(y=geneFamily,x=cluster,fill=geneIsCorrelated)) +
#     geom_tile()+
