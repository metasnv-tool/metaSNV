...
if( ! sampleIDColumnName %in% colnames(phenoDf) ){
  stop("Missing expected ID column named:",sampleIDColumnName)
}

if(!is.null(categoriesToShow) || 
   length(categoriesToShow) > 0 ){
  categoriesToShow <- intersect(categoriesToShow, colnames(phenoDf))
  if(length(categoriesToShow) == 0){
    warning("Column names specified for metadata analysis do not match metadata file.",
            "Using any possible metadata values instead")
  }
}

if(is.null(categoriesToShow) || 
   length(categoriesToShow) == 0){
  colsOkToTest <- apply(phenoDf,2,FUN=function(x){length(unique(x)) <= maxNLevels})
  categoriesToShow <- colnames(phenoDf[colsOkToTest])
  categoriesToShow <- categoriesToShow[categoriesToShow!=sampleIDColumnName] 
}else
if(is.null(categoriesToShow) || length(categoriesToShow) == 0){
  stop("No categorical metadata to analyse.")
}
```

Analysing categories (max number of values in each category is `r maxNLevels`): 

```{r}
paste(categoriesToShow,collapse=",")
```


```{r}
phenoDfToTest <- phenoDf %>% 
  rename(sampleName = !!sampleIDColumnName) %>% 
...
Number of samples total in phenotype data file: 
...
if(!is.null(sampleExtension) && 
   !is.na(sampleExtension) && 
   sampleExtension != ""){
...
Number of samples with subspecies assignments: 

```{r}
subSpecSamps$sampleName %>% unique() %>% length()
```
...
Missing metadata values for these samples:

```{r}
allData %>% filter(is.na(metadataValue) | is.na(value))
```

Plot of metadata values and clusters:

```{r,fig.width=10}
getMetadataOverviewPlot <- function(df){
p <- df %>% 
  filter(!is.na(metadataValue)) %>% 
...
if(!is.null(sampleExtension) && !is.na(sampleExtension) && sampleExtension != ""){
